<template>
  <div style="height: 100%;" class="container">
    ...
    <el-tabs v-model="activeName" @tab-click="handleTabClick">
      <el-tab-pane label="一维数组" name="single"></el-tab-pane>
      <el-tab-pane label="二维数组" name="dua"></el-tab-pane>
      <el-tab-pane label="对象数组" name="arrayObject"></el-tab-pane>
    </el-tabs>
    <div class="data-render" style="overflow-x: scroll;" v-html="renderData"></div>
    <el-tree :data="data" show-checkbox node-key="id" default-expand-all :expand-on-click-node="false">
      <span class="custom-tree-node" slot-scope="{ node, data }">
        <span style="display: inline-block;" v-html="node.label" v-if="!data.flag" class="label"></span>
        <input
        ...
        <span style="margin-left: 30px;">
          <template v-if="!data.flag">
            <el-button type="text" size="mini" v-if="!data.children" @click="() => modify(node, data)">
              Modify
            </el-button>
          ...
          <template v-else>
            <el-button type="text" size="mini" @click="() => changeData(node, data)" style="color: #ff9089;">
              Right <i class="el-icon-check"></i>
            </el-button>

            <el-button type="text" size="mini" @click="() => handleCancel(node, data)" style="color: #ff9089;">
  ...
  data() {
    return {
      ...
      defaultArrData: {
        ...
        dua: [
          ['Jacky', 'Alice', 'Tom', 'Dan', 'Michael'],
          ['Jacky', 'Alice', 'Tom', 'Dan', 'Michael'].map((item) => `Lil-${item}`),
        ],
        arrayObject: [
          {
            name: 'Jacky',
  ...
  methods: {
        ...
        .map((item) => {
          if (typeof item === 'object') {
            if (item instanceof Array) {
              ...
              return `<div class="pjfont" style="display: inline-block;margin-right: 6px;overflow-x: scroll;white-space: nowrap"> ${JSON.stringify(
                item,
              )} </div>`;
            }
          } else {
    ...
    modify(node, data) {
      ...
      if (this.activeName === 'single') {
        ...
        this.data[parIndex].children.splice(childIndex, 1, {
          id: data.id,
          flag: 2,
          keyLabel: keyValue,
